...
export function HomePageClient() {
  ...
  const handleCampusSelect = (id: CampusId) => {
    setAutoSelectionDone(true);
    toggleCampus(id);
    clearFocusStation();
  };

  const handleStationSelect = (station: StationRecord) => {
  ...
  useEffect(() => {
    const timer = trackingHighlightTimer.current;
    return () => {
      if (timer) {
  ...
  const triggerTrackingHighlight = () => {
